...
import com.cj.annotations.OPTION_MODULE_NAME
...
class ModuleBusProcessor : AbstractProcessor() {
    ...
    override fun process(
        ...
        if (elements.size > 1) {
            log_e(messager,"@${ModuleEventCenter::class.java.name}注解在${moduleName}中使用了多次，请去除重复注解，保留一个即可")
            return false
        }
        ...
        for (element in elements) {
                        ...
                        for (mirror in annotationMirrors) {
                            ...
                            if (annotationType == EventRegister::class.java.canonicalName) {
                                ...
                                for (entry in mirror.elementValues.entries) {
                                    if (entry.key.simpleName.toString() == "type") {
                                        try {
                                            var value = entry.value
                                            var className = value.value.toString()
                                            var method = EventRegisterEntity(
            ...
            for (eventRegisterEntity in methodList) {
                ...
                try {
                    ...
                    //泛型类型
                    val type = when ("${packageName}.${simpleName}") {
                        //java基础数据类型转换
                    ...
                    //Observable<type>
                    val observable =
                        ClassName("com.cj.base_common.bus", "Observable").parameterizedBy(type)

                    var funSpec: FunSpec =
                        FunSpec.builder("Gen_" + eventRegisterEntity.fieldName + "_Method")
            ...
            try {
                ...
                //不同的电脑和项目下，该路径不同，请自行避免报错
                var dirArray =
                    generatedDir!!.trim().replaceFirst("/", "").split("/").toMutableList()
                //dirArray[dirArray.size - 2] = "kapt"
                for((index,varPath)in dirArray.withIndex()){
                    if(varPath=="build"){
                        dirArray[index-1]="base_common"
                    }
                }
                //dirArray[dirArray.size - 6] = "base_common"
