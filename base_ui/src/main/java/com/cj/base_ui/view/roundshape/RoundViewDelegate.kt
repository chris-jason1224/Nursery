...
class RoundViewDelegate(view: View, context: Context, attrs: AttributeSet?) {
    ...
    private fun obtainAttributes(context: Context, attrs: AttributeSet?) {
        ...
        var ta: TypedArray =
            context.obtainStyledAttributes(attrs, R.styleable.base_ui_RoundTextView)
        //背景色
        backgroundColor = ta.getColor(
            R.styleable.base_ui_RoundTextView_base_ui_rv_backgroundColor,
            Color.TRANSPARENT
        )
        //按压背景色
        backgroundPressColor = ta.getColor(
            R.styleable.base_ui_RoundTextView_base_ui_rv_backgroundPressColor,
            Integer.MAX_VALUE
        )
        //圆角
        cornerRadius =
            ta.getDimensionPixelSize(R.styleable.base_ui_RoundTextView_base_ui_rv_cornerRadius, 0)
        //边框宽度
        strokeWidth =
            ta.getDimensionPixelSize(R.styleable.base_ui_RoundTextView_base_ui_rv_strokeWidth, 0)
        //边框颜色
        strokeColor =
            ta.getColor(R.styleable.base_ui_RoundTextView_base_ui_rv_strokeColor, Color.TRANSPARENT)
        //按压边框颜色
        strokePressColor = ta.getColor(
            R.styleable.base_ui_RoundTextView_base_ui_rv_strokePressColor,
            Integer.MAX_VALUE
        )
        //文字按压颜色
        textPressColor = ta.getColor(
            R.styleable.base_ui_RoundTextView_base_ui_rv_textPressColor,
            Integer.MAX_VALUE
        )

        isRadiusHalfHeight =
            ta.getBoolean(R.styleable.base_ui_RoundTextView_base_ui_rv_isRadiusHalfHeight, false)
        isWidthHeightEqual =
            ta.getBoolean(R.styleable.base_ui_RoundTextView_base_ui_rv_isWidthHeightEqual, false)
        cornerRadius_TL = ta.getDimensionPixelSize(
            R.styleable.base_ui_RoundTextView_base_ui_rv_cornerRadius_TL,
            0
        )
        cornerRadius_TR = ta.getDimensionPixelSize(
            R.styleable.base_ui_RoundTextView_base_ui_rv_cornerRadius_TR,
            0
        )
        cornerRadius_BL = ta.getDimensionPixelSize(
            R.styleable.base_ui_RoundTextView_base_ui_rv_cornerRadius_BL,
            0
        )
        cornerRadius_BR = ta.getDimensionPixelSize(
            R.styleable.base_ui_RoundTextView_base_ui_rv_cornerRadius_BR,
            0
        )
        //是否启用水波纹动效
        isRippleEnable =
            ta.getBoolean(R.styleable.base_ui_RoundTextView_base_ui_rv_isRippleEnable, true)
